	...
	if (op == "new") {
		...
		MongoClient.connect(uri, function(err, db) {
			...
			dbo.collection("User_Info").find(myObj).toArray(function(err, result) {
				...
				if (result.length != 0) {
					db.close();
					res.writeHead(301, {'Location': 'https://Qijin-Chau.github.io/GetBit/SignUp.html?success=false&reason=bad_acct'})
					res.end();
				} else {
					...
					dbo.collection("User_Info").insertOne(myObj, function(err, resp) {
						if (err) {
							res.writeHead(301, {'Location': 'https://Qijin-Chau.github.io/GetBit/SignUp.html?success=false&reason=insert_fail'});
							throw err;
						}
						console.log("New User Created");
						db.close();
						res.writeHead(301,{'Location':'https://Qijin-Chau.github.io/GetBit/Account.html?success=true&username=' + username + '&password=' + password});
						res.end();
					});
				}
			});
